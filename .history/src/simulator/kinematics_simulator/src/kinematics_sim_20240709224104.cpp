...
Eigen::Vector3d guassRandom3d(double std) {
  return std * Eigen::Vector3d(distribution(generator), distribution(generator),
                               distribution(generator));
}

double guassRandom(double std) { return std * distribution(generator); }
...
void simCallback(const ros::TimerEvent &e) {
  ...
  if (rcv_traj) {
    ...
    double segment_start_time =
        newest_trajectory.singul_traj[segment_Id].start_time;
    double segment_end_time =
        newest_trajectory.singul_traj[segment_Id].end_time;
    double pt_time;
    ...
    angul_vel =
        (ddsigma.transpose() * B_h * dsigma)(0, 0) / dsigma.squaredNorm();
